# Aacharya Meet — Academy Edition 1.1

A self-hostable small-group video calling app branded **Aacharya Meet / Aacharya Learning Academy**. React + Vite, Express, Socket.IO and browser WebRTC. Deploy the frontend and signaling server together on **one Render Free Web Service**. No paid disk, database or conferencing subscription is required to start.

This is a functional first release, **not a finished replacement for all paid Google Meet capabilities**. This edition uses academy-focused branding, an original book-and-A monogram, ivory/teal light mode and navy/teal dark mode. Theme selection is available on every screen and saved in local storage; first visits follow the system preference. The Academy Portal navigation tab opens https://aacharyalearningacademytm.onrender.com in a new tab so an ongoing call stays open. It is an external link, not an account or data integration. The official academy logo was not supplied.

## Start here: Render FREE deployment

1. Extract this ZIP. Create a new GitHub repository and upload the **contents of the extracted project folder**. `package.json`, `package-lock.json` and `render.yaml` must be at the repository root. Include `public/`, `src/`, `server/`, `tests/`, `index.html`, `vite.config.js` and the other project files. Do not upload `node_modules/` or a real `.env` file.
2. In Render choose **New → Web Service**, connect that repository, and select **Node** runtime.
3. Use these values:

   | Field | Value |
   |---|---|
   | Name | `aacharya-meet` (or any available name) |
   | Root directory | blank if package.json is at repository root; otherwise `aacharya-meet` |
...
7. Optionally set `APP_ORIGIN` to the exact final URL, e.g. `https://aacharya-meet-xxxx.onrender.com`, without a trailing slash. Redeploying/restarting clears active rooms.
...
- Persistent dark/light themes on every screen, mobile layout, rate limits, same-origin checks, security headers, bounded message sizes and server-side role checks.
...
Manual checks before using with your students and colleagues: two physical devices on different networks; iOS/Android permissions; screen-sharing audio; recording playback and consent; host reconnection; TURN relay connectivity; restart/expiry messaging; exports. Screen capture and codecs differ by browser. Desktop Chrome/Edge is a useful starting point, not a blanket compatibility guarantee.
...
`src/style.css` — Academy theme and responsive layouts

`public/icon.svg` — replaceable original Aacharya Meet monogram
