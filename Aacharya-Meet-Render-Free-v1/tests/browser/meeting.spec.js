...
test('two real browser contexts: media, waiting room, chat, collaboration and end',async({browser,page})=>{
 ...
 await page.getByLabel('Your name',{exact:true}).fill('Academy Host');await page.getByRole('button',{name:'Start a meeting'}).click();
 ...
 await guest.getByRole('button',{name:'Chat',exact:true}).click();await guest.getByLabel('Chat message').fill('Academy test message');await guest.getByRole('button',{name:'Send message'}).click();await page.getByRole('button',{name:'Chat',exact:true}).click();await expect(page.getByText('Academy test message')).toBeVisible();
 ...
 await page.screenshot({path:'test-results/aacharya-meeting-desktop.png',fullPage:true});
 page.on('dialog',d=>d.accept());await page.getByRole('button',{name:'End for all',exact:true}).click();await expect(guest.getByRole('heading',{name:'Thank you for learning with us.'})).toBeVisible();
 expect(errors).toEqual([]);await context.close();
});
test('landing mobile fits the viewport',async({page})=>{await page.setViewportSize({width:390,height:844});await page.goto('/');await expect(page.getByRole('button',{name:'Start a meeting'})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);await page.screenshot({path:'test-results/aacharya-mobile.png',fullPage:true});});
